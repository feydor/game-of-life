...
const Canvas = (props) => {
  const init = async () => {
    ...
    document.getElementById(
      "pause"
    ).style.transform = `translate(-50%, -50%) translate(${3 * x}px,${y}px)`;
    document.getElementById(
      "reset"
    ).style.transform = `translate(-50%, -50%) translate(${5 * x}px,${y}px)`;
...
function render() {
  if (resizeRendererToDisplaySize(renderer)) {
    ...
    if (!globals.is3D) {
      globals.orthographicCamera.aspect =
        canvas.clientWidth / canvas.clientHeight;
      globals.orthographicCamera.updateProjectionMatrix();
    } else {
      globals.perspectiveCamera.aspect =
        canvas.clientWidth / canvas.clientHeight;
  ...
  if (
    globals.runState.isRunning &&
    Math.trunc(globals.clock.getDelta()) % globals.gameSpeed === 0
  ) {
    GameState.update();
    updateCells(false);
  }
...
function initCameras() {
  ...
  const near = 0.1;
  const far = 10000;
  globals.perspectiveCamera = new THREE.PerspectiveCamera(
    fov,
    aspect,
    near,
    far
  ...
  globals.orthographicCamera = new THREE.OrthographicCamera(
    -aspect,
    aspect,
    aspect,
    -aspect,
...
function init2d() {
  ...
  const gGrid = new THREE.GridHelper(
    size,
    divisions,
    colorCenterLine,
    colorGrid
...
function init3d() {
  const fieldGeom = new THREE.SphereGeometry(
    globals.boardSize,
    globals.boardSize,
    globals.boardSize
  );
  ...
  for (let y = 0; y < GameState.HEIGHT; y++) {
    for (let x = 0; x < GameState.WIDTH; x++) {
      ...
      let overflow = 0.5; // cell size / 2
      let coord = latLongToVector3(
        10 * (x - globals.boardSize / 2 + overflow),
        10 * (y - globals.boardSize / 2 + overflow),
        1,
        0.04
      );
      cell.translateOnAxis(coord, globals.boardSize);

      gCells3d[x + y * GameState.HEIGHT] = {
        geom: cell,
        material: cellMaterial,
        state: isAlive,
      };
...
function updateCells(reset) {
  ...
  let isAliveMaterial = globals.is3D
    ? materials.isAlive3d
    : materials.isAlive2d;
  ...
  for (let y = 0; y < GameState.HEIGHT; y++) {
    for (let x = 0; x < GameState.WIDTH; x++) {
      ...
      if (globals.colors && globals.trail) {
        if (newState) {
          const randColor =
            palette[Math.floor(Math.random() * Math.floor(palette.length))];
